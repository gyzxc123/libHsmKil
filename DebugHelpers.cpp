...
#define PRINT_TAG(tag)				printf("%s: ", tag)
#define LOG_PUTS(prio, tag, szLog)		PRINT_TAG(tag); puts(szLog)
#define LOG_PRINT(prio, tag, fmt, arg...)	PRINT_TAG(tag); printf(fmt, ## arg)
#define LOG_VPRINT(prio, tag, fmt, pArgs)	PRINT_TAG(tag); vprintf(fmt, pArgs)
#else
...
DBGAPI void ShowI2C(bool write, unsigned char i2c_addr, unsigned char reg, const unsigned char *buf, unsigned char len)
{
	pthread_mutex_lock(&LogProtectLevel1);
	PrintMicroSec(PRINT_LEVEL, LOG_TAG);
	DbgOut1(PRINT_LEVEL, LOG_TAG, "I2C %s %s%0x, reg=%02x (%02i) ", write ? "Write to" : "Read from", GetDeviceName(i2c_addr), i2c_addr, reg, reg);
	DbgOut1(PRINT_LEVEL, LOG_TAG, "len = %i%s", len, len>4 ? "\n" : ": ");
	for (int i=0; i<len; i++)
	{
		DbgOut1(PRINT_LEVEL, LOG_TAG, "%02x ", (unsigned int) *buf++);
	}
	DbgOut1(PRINT_LEVEL, LOG_TAG, "\n");
	pthread_mutex_unlock(&LogProtectLevel1);
}
